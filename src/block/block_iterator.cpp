...
BlockIterator::BlockIterator(std::shared_ptr<Block> b, const std::string &key,
  ...
  current_index = *b->get_idx_binary(key, tranc_id);
  this->tranc_id_ = tranc_id;
}
...
BlockIterator &BlockIterator::operator++() {
  ...
  ++current_index;
  this->tranc_id_ = block->get_tranc_id_at(current_index);
  cached_value = std::nullopt;
  return *this;
}
...
bool BlockIterator::operator==(const BlockIterator &other) const {
  ...
  return (this->current_index == other.current_index &&
          this->tranc_id_ == other.tranc_id_);
}
...
void BlockIterator::skip_by_tranc_id() {
  ...
  // ? 现在你不需要考虑这个函数
  while (!is_end()) {
    uint64_t current_tranc_id = block->get_tranc_id_at(current_index);
    // 如果当前位置的事务ID小于等于当前事务ID，说明对当前事务可见
